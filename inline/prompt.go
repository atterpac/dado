...
func SelectOption(prompt string, options []string, descriptions []string) int {
	...
	for {
		...
		fmt.Fprintf(out, "\033[%dA", len(options)+1)

		// Print prompt
		fmt.Fprintf(out, "\r\033[K  %s%s?%s %s%s%s\n", Bold, Magenta, Reset, Bold, prompt, Reset)

		// Print options
		for i, opt := range options {
			fmt.Fprint(out, "\r\033[K")
			if i == selected {
				fmt.Fprintf(out, "  %s%s▸%s %s%s%s", Bold, Cyan, Reset, Cyan, opt, Reset)
				if i < len(descriptions) && descriptions[i] != "" {
					fmt.Fprintf(out, " %s%s%s", Dim, descriptions[i], Reset)
				}
			} else {
				fmt.Fprintf(out, "    %s%s%s", White, opt, Reset)
				if i < len(descriptions) && descriptions[i] != "" {
					fmt.Fprintf(out, " %s%s%s", Dim, descriptions[i], Reset)
				}
			}
...
func selectOptionSimple(prompt string, options []string, descriptions []string) int {
	...
	for i, opt := range options {
		...
		fmt.Fprintf(out, "    %s%d.%s %s", Dim, i+1, Reset, opt)
		if desc != "" {
			fmt.Fprintf(out, " %s%s%s", Dim, desc, Reset)
		}
		fmt.Fprintln(out)
	}
